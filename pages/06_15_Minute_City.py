...
scientific_note(
    ...
    "A building is considered covered when at least one service of the selected category is "
...
ACCESS_COLOURS = {
    "Access 0": [214, 39, 40, 210],
    "Access 1": [127, 127, 127, 205],
}

DIVERSITY_COLOURS = {
    "Priority 0": [127, 127, 127, 190],
    "Priority 1": [214, 39, 40, 220],
}
...
with st.sidebar:
    ...
    analysis_mode = st.selectbox(
        ...
        [
            "All services",
            "All daily services (excluding hospitals)",
            "Selected service",
    ...
    if analysis_mode == "Selected service":
        selected_service_label = st.selectbox(
            "Service",
            list(SERVICE_COLUMNS),
            index=0,
        )
    elif analysis_mode == "Custom service combination":
        custom_service_labels = st.multiselect(
            "Services required",
            options=list(SERVICE_COLUMNS),
            default=list(DAILY_SERVICE_COLUMNS),
        )
...
if analysis_mode == "All services":
    scenario_label = "All services"
    scenario_fields = list(SERVICE_COLUMNS.values())

elif analysis_mode == "All daily services (excluding hospitals)":
    scenario_label = "All daily services, excluding hospitals"
    scenario_fields = list(DAILY_SERVICE_COLUMNS.values())

elif analysis_mode == "Selected service":
    scenario_label = str(selected_service_label)
    scenario_fields = [SERVICE_COLUMNS[str(selected_service_label)]]

else:
    if not custom_service_labels:
        st.warning("Select at least one service for the custom scenario.")
        st.stop()

    scenario_label = " + ".join(custom_service_labels)
    scenario_fields = [
        SERVICE_COLUMNS[label]
        for label in custom_service_labels
    ]

covered = coverage_for_services(filtered, scenario_fields)
...
with map_tab:
    ...
    st.caption(
        "Access 1 indicates that the building satisfies the selected coverage scenario; "
        "Access 0 indicates a coverage gap. The map uses the real Polygon or MultiPolygon "
        "building footprints stored in geometry_wkt."
    )
...
- **All services** reproduces the strict complete-coverage scenario.
- **All daily services, excluding hospitals** reflects the thesis discussion that hospitals are less suitable for routine proximity analysis because of their specialised and occasional use.
- **Selected service** isolates the coverage of hospitals, health centres, pharmacies or another service.
- **Custom service combination** allows an exploratory combination while preserving the same stored accessibility indicators.
