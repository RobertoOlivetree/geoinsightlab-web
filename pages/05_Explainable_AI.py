...
summary[1].metric("K-means clusters reproduced", len(cluster_classes))
...
with prediction_tab:
    st.subheader(f"K-means cluster reproduction — {selected_model_name}")
    ...
    if selected_map == "Original K-means clusters":
        map_column = "original_cluster"
        caption = (
            "Original K-means classification used as the reference target "
            "for the explanatory supervised models."
        )
    else:
        map_column = "reproduced_cluster"
        caption = (
            f"K-means cluster classification reproduced by the {selected_model_name}. "
            "This map is an auxiliary validation layer; the XAI contribution is provided "
            "by variable importance, decision rules and local explanations."
        )

    render_map(result, map_column, colours, maximum_polygons)
    st.caption(caption)

    distribution = (
        result["reproduced_cluster_label"]
        .value_counts()
        .rename_axis("Reproduced cluster")
        .reset_index(name="Buildings")
    )
    chart = px.bar(
        distribution,
        x="Reproduced cluster",
        y="Buildings",
        title=f"Distribution of K-means clusters reproduced by the {selected_model_name}",
    )
    ...
    if "original_cluster" in result.columns:
        ...
        st.metric(
            "Agreement with original K-means labels",
            f"{agreement.mean():.1%}",
        )
    ...
    if probabilities is not None:
        confidence = px.histogram(
            ...
            color="reproduced_cluster_label",
            nbins=35,
            opacity=0.75,
            title="Confidence in reproducing the K-means classification",
        )
        confidence.update_layout(
            bargap=0.04,
            legend_title="Reproduced cluster",
        )
...
with local_tab:
    st.subheader("Building-level explanation")
    ...
    cards[1].metric("Original K-means cluster", str(selected_record.get("original_cluster_label", "Not available")))
    cards[2].metric("Reproduced cluster", str(selected_record["reproduced_cluster_label"]))
    ...
    if probabilities is not None:
        ...
        probability_table = pd.DataFrame({
            "K-means cluster": [readable_profile(value, mapping) for value in raw_classes],
        ...
        probability_chart = px.bar(
            ...
            y="K-means cluster",
            orientation="h",
            title="Probabilities assigned to each K-means cluster",
        )
...
- LIME creates a local surrogate explanation for an individual building.
- SHAP is treated separately in the dedicated attractiveness-analysis module.
...
These explanations describe model behaviour and do not establish causal effects.
    ...
    download_columns = [field for field in [
        "osm_id", "designacao_simplificada", target_column,
        "original_cluster", "original_cluster_label",
        "reproduced_cluster", "reproduced_cluster_label",
        "prediction_confidence", *bundle.feature_columns,
        "latitude", "longitude",
