...
available_clusters = sorted(
    result[map_column].dropna().astype(int).unique().tolist()
)
for cluster in available_clusters:
    rgba = CLUSTER_COLOURS.get(
        f"Cluster {cluster}",
        PROFILE_COLOURS[cluster % len(PROFILE_COLOURS)],
    )
    hex_colour = "#{:02x}{:02x}{:02x}".format(*rgba[:3])
    legend_items.append(
        f"<span style='display:inline-flex;align-items:center;margin-right:18px;'>"
        f"<span style='width:14px;height:14px;background:{hex_colour};"
        f"display:inline-block;border-radius:3px;margin-right:6px;'></span>"
        f"Cluster {cluster}</span>"
    )
