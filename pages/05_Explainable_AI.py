...
page_header(
    "Explainable AI",
    "Explore how Decision Trees, Random Forest and LIME reproduce and interpret the spatial profiles identified by K-means.",
)
scientific_note(
...
CLUSTER_COLOURS = {
    "Cluster 0": [46, 139, 87, 210],
    "Cluster 1": [255, 165, 0, 210],
    "Cluster 2": [65, 105, 225, 210],
    "Cluster 3": [186, 85, 211, 210],
}
...
def local_effects(bundle: ModelBundle, row: pd.DataFrame, reference: pd.DataFrame, predicted: str) -> pd.DataFrame:
    ...
    baseline = float(
        bundle.model.predict_proba(
            transform(bundle, row)
        )[0, position]
    )

    rows = []
    for field in bundle.feature_columns:
        building_value = float(row.iloc[0][field])
        candidates = [
            float(reference[field].quantile(0.25)),
            float(reference[field].median()),
            float(reference[field].quantile(0.75)),
        ]

        best_value = building_value
        best_effect = 0.0

        for candidate in candidates:
            modified = row[bundle.feature_columns].copy()
            modified.loc[modified.index[0], field] = candidate
            probability = float(
                bundle.model.predict_proba(
                    transform(bundle, modified)
                )[0, position]
            )
            effect = baseline - probability
        ...
        rows.append(
            {
                ...
                "Perturbed value": best_value,
                "Local contribution": best_effect,
            }
        )
...
The K-means labels provide the reference classification used in the doctoral research.
Decision Trees and Random Forest are trained as explanatory surrogate models to
reproduce the clusters and reveal their structure. LIME then explains the
classification assigned to an individual building.
"""
)
...
with local_columns[0]:
    ...
    if effects.empty:
        ...
        effects["Local contribution"].abs().max(),
        0.0,
        atol=1e-10,
    ):
        st.info(
            "The selected model is locally stable for this building: plausible "
            "single-variable perturbations do not change its predicted probability."
        )
    else:
        effects_chart = px.bar(
            ...
            y="Feature",
            orientation="h",
            hover_data=["Building value", "Perturbed value"],
            title="Sensitivity to plausible single-variable changes",
        )
...
st.caption(
    "Compare the original K-means clusters with the clusters reproduced by the "
...
def render_coloured_cluster_map(
    ...
    """Render real building polygons using the shared clustering-map utility."""
    if render_categorical_polygon_map is None:
        st.info("The categorical polygon-map utility is unavailable.")
        return
