...
page_header(
    "Explainable AI",
    "Understand why a building is assigned to a specific urban profile and which variables drive that prediction.",
)
scientific_note(
    "The page loads pretrained models when available and can train temporary models from the current dataset for testing."
)
...
TARGET_ALIASES = ["urban_profile", "perfil_urbano", "cluster_label", "cluster", "Cluster", "profile"]
DEFAULT_FEATURES = [
    "Bancos",
    "Centro Saude",
    "Farmacias",
...
OPTIONAL_FEATURES = ["numero_servicos_proximos", "numero_categorias_proximas"]
DISPLAY_LABELS = {
    "Bancos": "Banks",
...
if target_column is not None:
    prepared_target = data[target_column].map(normalise_class).map(lambda value: readable_profile(value, mapping))
...
if prepared_target is not None:
    result["observed_profile"] = prepared_target
...
The models reproduce a building-level urban-profile classification from socio-spatial accessibility and demographic variables. The page combines prediction with global and local explanation.

### Model artefacts

The application first searches the `models` directory for pretrained Decision Tree, Random Forest and XGBoost artefacts. It also searches for a fitted scaler or preprocessing object. The expected feature order and class labels should be recorded in `model_metadata.json`.
