...
page_header(
    "Explainable AI",
    "Explain how Decision Trees, Random Forest and LIME reproduce and interpret the clusters identified by K-means.",
)
scientific_note(
    "The K-means labels are the reference classification. Supervised models are used as explanatory surrogates to reveal global variable importance, decision rules and local building-level explanations."
)
...
# Variables used to explain the K-means clustering in the thesis workflow.
# Only fields that exist in the loaded dataset are retained.
DEFAULT_FEATURES = [
...
def train_models(features: pd.DataFrame, target: pd.Series, test_size: float, depth: int, trees: int) -> dict[str, ModelBundle]:
    ...
    estimators: dict[str, Any] = {
        ...
        "Random Forest": RandomForestClassifier(
            n_estimators=trees, min_samples_leaf=3, n_jobs=-1, random_state=RANDOM_STATE
        ),
    }
...
if not pretrained and allow_training and prepared_target is not None:
    ...
    target = prepared_target.loc[valid].astype(str).copy()
...
with st.sidebar:
    selected_model_name = st.selectbox("Explanatory model", list(bundles), index=min(1, len(bundles) - 1))
...
result["reproduced_cluster"] = pd.to_numeric(
    pd.Series(raw_predictions, index=result.index),
    errors="coerce",
).astype("Int64")

result["reproduced_cluster_label"] = [
    readable_profile(value, mapping) for value in raw_predictions
]

if prepared_target is not None:
    result["original_cluster"] = pd.to_numeric(
        prepared_target,
        errors="coerce",
    ).astype("Int64")
    result["original_cluster_label"] = prepared_target.map(
        lambda value: readable_profile(value, mapping)
    )

result["prediction_confidence"] = (
    probabilities.max(axis=1) if probabilities is not None else np.nan
)

cluster_classes = sorted(
    result["reproduced_cluster"].dropna().astype(int).unique().tolist()
)
colours = {
    cluster: PROFILE_COLOURS[index % len(PROFILE_COLOURS)]
    for index, cluster in enumerate(cluster_classes)
}

summary = st.columns(5)
summary[0].metric("Buildings", f"{len(result):,}")
summary[1].metric("Reference clusters", result["original_cluster"].nunique() if "original_cluster" in result.columns else "N/A")
summary[2].metric("Explanatory model", selected_model_name)
summary[3].metric("Mean confidence", f"{result['prediction_confidence'].mean():.3f}" if probabilities is not None else "N/A")
if "original_cluster" in result.columns:
    agreement = (
        result["original_cluster"].astype("Int64")
        == result["reproduced_cluster"].astype("Int64")
    )
    summary[4].metric("Agreement with K-means", f"{agreement.mean():.1%}")
else:
    summary[4].metric("Agreement with K-means", "N/A")

st.markdown(
    """
### Thesis workflow

The K-means labels are the reference classification. Decision Trees and Random Forest
models are trained as explanatory surrogate models: they reproduce the clusters and
make their structure interpretable. LIME then explains the classification of an
individual building.
"""
)
...
# ---------------------------------------------------------------------------
st.divider()
st.header("1. Global explanation of the K-means clusters")

global_model_name = st.selectbox(
    "Model used for global variable importance",
    list(bundles),
    index=list(bundles).index("Random Forest") if "Random Forest" in bundles else 0,
    key="global_model",
)
global_bundle = bundles[global_model_name]

importance = importance_table(global_bundle)
if importance.empty:
    ...
    chart = px.bar(
        ...
        y="Feature",
        orientation="h",
        title=f"Contribution of variables to reproducing the K-means clusters — {global_model_name}",
    )
    chart.update_layout(xaxis_tickformat=".0%")
...
st.caption(
    "The importance values describe how the explanatory model reproduces the K-means "
    "classification. They do not establish causal effects."
)
...
# ---------------------------------------------------------------------------
st.divider()
st.header("2. Decision-tree rules")

tree_bundle = bundles.get("Decision Tree")
if tree_bundle is None:
    st.info("The Decision Tree is unavailable in the current session.")
else:
    estimator = tree_bundle.model
    ...
    if isinstance(estimator, DecisionTreeClassifier):
        rules_text = export_text(
            estimator,
            feature_names=[display_label(field) for field in tree_bundle.feature_columns],
            decimals=3,
        )
        with st.expander("View the complete decision-tree rules", expanded=True):
            st.code(rules_text, language="text")
    else:
        st.info("Decision-tree rules could not be extracted.")

# ---------------------------------------------------------------------------
# 3. LOCAL EXPLANATION
# ---------------------------------------------------------------------------
st.divider()
st.header("3. Local explanation of an individual building")

identifier = next(
    (field for field in ("osm_id", "building_id", "id") if field in result.columns),
    None,
)
...
cards[1].metric(
    "Original K-means cluster",
    str(selected_record.get("original_cluster_label", "Not available")),
)
cards[2].metric(
    "Cluster reproduced by the model",
    str(selected_record["reproduced_cluster_label"]),
)
cards[3].metric(
    "Confidence",
    f"{float(selected_record['prediction_confidence']):.1%}"
...
if probabilities is not None:
    ...
    probability_table = pd.DataFrame(
        {
            "K-means cluster": [
                readable_profile(value, mapping) for value in raw_classes
            ],
    ...
    probability_chart = px.bar(
        ...
        y="K-means cluster",
        orientation="h",
        title="Probability assigned to each K-means cluster",
    )
...
with local_columns[0]:
    st.subheader("Local perturbation")
    ...
    if effects.empty:
        st.info("Local probability perturbations are unavailable.")
    else:
        effects_chart = px.bar(
            ...
            title="Effect of replacing each variable with its median",
        )
        effects_chart.add_vline(x=0, line_width=1)
        st.plotly_chart(effects_chart, width="stretch")

with local_columns[1]:
    st.subheader("LIME")
    if not LIME_AVAILABLE:
        st.info("Add `lime` to requirements.txt to activate LIME.")
    else:
        try:
            lime_table = lime_values(
                bundle,
                selected_row,
                reference,
                raw_predictions[position],
            ...
            st.warning(f"LIME could not be generated: {error}")

        if lime_table.empty:
            st.info("No LIME explanation was produced for this building.")
        else:
            lime_chart = px.bar(
                lime_table.sort_values("Contribution", ascending=True),
                x="Contribution",
                y="Condition",
                orientation="h",
                title="LIME explanation for the selected building",
            )
            lime_chart.add_vline(x=0, line_width=1)
            st.plotly_chart(lime_chart, width="stretch")
            st.dataframe(lime_table.round(4), width="stretch", hide_index=True)

if tree_bundle is not None:
    st.subheader("Decision path for the selected building")
    path_lines = decision_path(tree_bundle, selected_row)
    if path_lines:
        for number, line in enumerate(path_lines, 1):
            st.markdown(f"**{number}.** {line}")
    else:
        st.info("A decision path could not be extracted.")

# ---------------------------------------------------------------------------
# 4. SPATIAL VALIDATION MAP
# ---------------------------------------------------------------------------
st.divider()
st.header("4. Spatial validation")

st.caption(
    "The map is an auxiliary validation output. The core XAI results are the global "
    "importance, decision rules and local LIME explanation shown above."
)

def render_coloured_cluster_map(
    dataframe: pd.DataFrame,
    cluster_column: str,
    maximum: int,
) -> None:
    """Render coloured building polygons directly from WKT geometries."""
    geometry_field = next(
        (field for field in ("geometry_wkt", "geometry") if field in dataframe.columns),
        None,
    )
    if geometry_field is None:
        st.info("No polygon geometry is available for mapping.")
        return

    sample = dataframe.loc[
        dataframe[cluster_column].notna() & dataframe[geometry_field].notna()
    ].copy()

    if len(sample) > maximum:
        sample = sample.sample(maximum, random_state=RANDOM_STATE)

    palette = {
        int(cluster): PROFILE_COLOURS[index % len(PROFILE_COLOURS)]
        for index, cluster in enumerate(
            sorted(sample[cluster_column].dropna().astype(int).unique())
        )
    }
    ...
    for _, record in sample.iterrows():
        try:
            geometry = (
                shapely_wkt.loads(record[geometry_field])
                if isinstance(record[geometry_field], str)
                else record[geometry_field]
            )
            cluster = int(record[cluster_column])
    ...
    if not features_geojson:
        st.info("No valid building polygons could be prepared for the map.")
        return
    ...
    view_state = pydeck.ViewState(
        latitude=float(sample["latitude"].median()),
        longitude=float(sample["longitude"].median()),
        zoom=12,
        pitch=0,
    )
    ...
    deck = pydeck.Deck(
        ...
        map_style="dark",
        tooltip={
            "html": "<b>{cluster_label}</b><br/>OSM ID: {osm_id}",
            "style": {"backgroundColor": "#111827", "color": "white"},
        },
    )
    st.pydeck_chart(deck, use_container_width=True)

map_options = ["Reproduced clusters"]
if "original_cluster" in result.columns:
    map_options.insert(0, "Original K-means clusters")
...
map_column = (
    "original_cluster"
    if selected_map == "Original K-means clusters"
    else "reproduced_cluster"
)
render_coloured_cluster_map(result, map_column, maximum_polygons)

distribution_columns = st.columns(2)

with distribution_columns[0]:
    original_distribution = (
        result["original_cluster_label"].value_counts()
        if "original_cluster_label" in result.columns
        else pd.Series(dtype=int)
    )
    if not original_distribution.empty:
        original_frame = original_distribution.rename_axis("Cluster").reset_index(name="Buildings")
        st.plotly_chart(
            px.bar(
                original_frame,
                x="Cluster",
                y="Buildings",
                title="Original K-means cluster distribution",
            ),
            width="stretch",
        )
...
with distribution_columns[1]:
    ...
    st.plotly_chart(
        px.bar(
            ...
            x="Cluster",
            y="Buildings",
            title=f"Clusters reproduced by the {selected_model_name}",
        ),
        width="stretch",
    )

# ---------------------------------------------------------------------------
# 5. MODEL COMPARISON AND METHODOLOGY
# ---------------------------------------------------------------------------
st.divider()
with st.expander("Model comparison"):
    ...
    if not rows:
        st.info("Comparable test metrics are unavailable.")
    else:
        comparison_table = pd.DataFrame(rows).sort_values("F1-score", ascending=False)
        st.dataframe(comparison_table.round(3), width="stretch", hide_index=True)

        long_table = comparison_table.melt(
            id_vars="Model",
            var_name="Metric",
            value_name="Score",
        )
        comparison_chart = px.bar(
            ...
            color="Metric",
            barmode="group",
            title="Predictive reproduction of the K-means labels",
        )
        comparison_chart.update_layout(yaxis_range=[0, 1])
        st.plotly_chart(comparison_chart, width="stretch")

with st.expander("Methodology and interpretation limits"):
    st.markdown(
        """
- K-means provides the original unsupervised cluster labels.
- Decision Trees and Random Forest reproduce those labels as explanatory surrogate models.
- Global importance identifies the variables most used by the surrogate model.
- Decision-tree rules expose interpretable thresholds.
- LIME provides a local explanation for one building.
- The results explain model behaviour and do not establish causal relationships.
- SHAP is addressed separately in the dedicated urban-attractiveness module.
"""
    )
        ...
        pd.DataFrame(
            {
                ...
                "Indicator": [
                    display_label(field) for field in bundle.feature_columns
                ],
            }
        ),
        width="stretch",
